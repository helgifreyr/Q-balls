c  
      ...
      PARAMETER (NX= 151 , NY= 50  , NK= 1 , N=NX*NY ,LOUT=6, NXMAX=200,
...
c UUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUU       
	 ...
	 DOUBLE PRECISION  c1,c2,c3,eta,nr,w,con,g,
...
C
      COMMON /FDSCON/   c1,c2,c3,eta,nr,w,con,g
...
c this is winding number
c               nr = 1.d0
                read(*,*) nr

c this is the frequency
               w=1.d0  
...
c the potential here: c1 phi^6+c2 phi^4+c3 phi^2
c notation in Kunz e-Print: gr-qc/0505143  

c these are the parameters in the potentiel
          c1=1.d0
          c2=-2.d0
          c3=1.1d0
...
C**                                                             ***
     ...
     & U8yy,U9yy,U10yy,U11yy,U12yy
       Double precision sn,cs,csc,ct,r,T44a,T35a,T53a,ff  
...
c			  write(6,*)U1,try
	...
	ff=1.d0-X(I)
      ...
      eq1= -(nr**2*U1) - c3*r**2*sn**2*U1 - 2*c2*r**2*sn**2*U1**3 - 
     -  3*c1*r**2*sn**2*U1**5 + (2*ff**2*r*sn**2*U1x)/con + 
     -  r**2*sn**2*((-2*ff**3*U1x)/con**2 + 
     -     (ff**4*U1xx)/con**2) + cs*sn*U1y + sn**2*U1yy + 
     -  r**2*sn**2*U1*w**2
...
c      write(2,*)r, U1xx  
 	...
 	En1(I) = (csc**2*nr**2*U1**2)/r**2 
     &  + (ff**4*U1x**2)/con**2 + U1y**2/r**2

      En2(I)= U1**2*w**2

      En3(I)=U1**2*(c3 + c2*U1**2 + c1*U1**4)
 
        
	ro=En1(I)+En2(I)+En3(I)
...
C**                                                             ***     
     ...
     &               PUT(MT),PUX(MV),PUXX(MV),PUY(MV),PUYY(MV),max 
       Double precision sn,cs,csc,ct,r,z,ff,f
...
c     ICOM  NUMBER OF THE SOLUTION COMPONENT
   ...
   50 continue  
	...
	ff=1-X(I)
      ...
      Vyy11=sn**2
      Vx11=(2*ff**2*r*sn**2)/con - (2*ff**3*r**2*sn**2)/con**2
      Vy11=cs*sn
      V11=-nr**2 - r**2*sn**2*(c3 + 6*c2*U1**2 + 15*c1*U1**4 - w**2)
