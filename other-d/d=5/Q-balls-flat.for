c  9 NOV 13 
c Q-balls in Minkowski st background d=5
...
c			  write(6,*)U1,try
	...
	ff=1.d0-X(I)
      ...
      eq1= -1.*cs*nr**2*U1 - 1.*cs*r**2*sn**2*U1*
     -   (c3 + 2.*c2*U1**2 + 3.*c1*U1**4) + 
     -  (3.*cs*ff**2*r*sn**2*U1x)/con + 
     -  cs*r**2*sn**2*((-2.*ff**3*U1x)/con**2 + 
     -     (ff**4*U1xx)/con**2) + cs2*sn*U1y + cs*sn**2*U1yy + 
     -  cs*r**2*sn**2*U1*w**2  
...
c     ICOM  NUMBER OF THE SOLUTION COMPONENT
   ...
   50 continue  
	...
	ff=1-X(I)
       ...
       Vxx11=(cs*ff**4*r**2*sn**2)/con**2
       Vxy11=0
       Vyy11=cs*sn**2
       Vx11=(cs*ff**2*r*(3*con - 2*ff*r)*sn**2)/con**2
       Vy11=cs2*sn
       V11=
     & -(cs*(nr**2 + r**2*sn**2*
