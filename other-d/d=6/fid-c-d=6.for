c  9 NOV 13 
c Q-balls in Minkowski st background  d= 6
...
c           w = 0.991d0+(ii-1)*0.001d0 

c             c2 = -0.55d0+(ii-1)*0.01d0

c       c1 = 0.05d0-(ii-1)*0.01d0 
...
c			  write(6,*)U1,try
	...
	ff=1.d0-X(I)
     ...
     -  4.*cs**2*ff**2*r*sn*U1x + 
     -  cs**2*r**2*sn*(-2.*ff**3*U1x + ff**4*U1xx) + 
     -  0.5*cs*(-1. + 3.*cs2)*U1y + cs**2*sn*U1yy + 
     -  cs**2*r**2*sn*U1*w**2
...
c     ICOM  NUMBER OF THE SOLUTION COMPONENT
   ...
   50 continue  
	...
	ff=1-X(I)
       ...
       Vxx11=cs**2*ff**4*r**2*sn
       Vxy11=0
       Vyy11=cs**2*sn
       Vx11=-2*cs**2*ff**2*r*(-2 + ff*r)*sn
       Vy11=(cs*(-1 + 3*cs2))/2.
       V11=
     & -(cs*(ct*nr**2 + cs*r**2*sn*
     &        (c3 + 6*c2*U1**2 + 15*c1*U1**4 - w**2)))
