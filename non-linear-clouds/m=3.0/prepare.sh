...
cp ../../d=4/m=3.0/w=$1/funct.dat w=$1/1st/rh=0.001/
...
cp ../script/* .
./run.sh 0.001 $1
mkdir ../rh=0.010 && cd ../rh=0.010
cp ../rh=0.001/funct.dat .
python ../ftoff.py
cp ../script/* .
./run.sh 0.010 $1
cd ..
python keyrsla.py 0.020 $2 0.010
cd rh=$2
# first two are for $2=X.XXXX and the second two are for $2=X.XX5
# changed to 10.0/*10 since step is 0.010 not 0.005
var1=$( echo "$2*1000.0/10.0*10" | bc)
...
./run.sh $var2 $1
cd ..
python keyrsla.py $3 $var2 0.010
