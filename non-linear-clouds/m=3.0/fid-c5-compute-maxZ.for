...
c     new version with FDS01 

      PARAMETER (NX= 251 , NY= 30  , NK= 1 , N=NX*NY ,LOUT=6, NXMAX=300,
     &           NOX= 6 , NOY= 6  , MNOX= 10 , MNOY= 10,
...
c UUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUU       
	 ...
	 DOUBLE PRECISION  c1,c2,c3,nr,w,ct,rh,
     & x,y,k1,k2,k3,k4,fi1,fi2,a1,r,OmegaH
 

      LOGICAL  LINFO(MLI),LLIN(MD)                                              
      EXTERNAL  U01E1,U02E1,U03E1,U04E1,FDS500
C
      COMMON /FDSCON/   c1,c2,c3,nr,w,ct,rh                               
...
c       c3 = lambda b
	...
	OmegaH=Sqrt(ct*(ct - 1.*rh))/(2.*ct**2 - 3.*ct*rh + rh**2)
...
c       w = 0.99d0-(ii-1)*0.01d0

c             c2 =  -0.09d0+(ii-1)*0.01d0 

c          c1 =  0.59d0-(ii-1)*0.01d0 
...
c	      rewind(5)
	       open(unit=5,File='res.txt')
	        WRITE(5,489) c1,c2,c3,nr,w,rh,ct
	        Close(Unit=5)
...
C**                                                             ***
     ...
     & U8yy,U9yy,U10yy,U11yy,U12yy
       Double precision sn,cs,csc,r,T44,T34,T53a,ff  
	 ...
	 Double precision norm,rn,xn,max, ro,try,
     &  g,H,expr1,expr2,expr3,expr4,expr5,fW,em2F0,em2F1,em2F2,sqr
c
C**                                                             ***
C**-----------------------------------------------------------------***
C**                                                             ***
...
c this is \sqr(-g): metric determinant

	sqr=  r*((-1. + ct/Sqrt(r**2 + rh**2))**2 + 
     -    (ct*(ct - 1.*rh)*cs**2)/(r**2 + rh**2))*Sqrt(g)*
     -  sn
...
c this is e^(-2F0)
	...
	em2F0=        (((ct*(ct - 1.*rh))/(r**2 + rh**2) + 
     -       (-1. + ct/Sqrt(r**2 + rh**2))**2)**2 + 
     -    (ct*(-1.*ct + rh)*(-1.*rh + Sqrt(r**2 + rh**2))*
     -       Sin(t)**2)/(r**2 + rh**2)**1.5)/
     -  ((-1. + ct/Sqrt(r**2 + rh**2))**2 + 
     -    (ct*(ct - 1.*rh)*Cos(t)**2)/(r**2 + rh**2))

ccccccccccccccccccccccccccccccccccccccccccccccccc

	em2F1=  1./((-1. + ct/Sqrt(r**2 + rh**2))**2 + 
     -    (ct*(ct - 1.*rh)*Cos(t)**2)/(r**2 + rh**2))

	em2F2=        ((-1. + ct/Sqrt(r**2 + rh**2))**2 + 
     -    (ct*(ct - 1.*rh)*Cos(t)**2)/(r**2 + rh**2))/
     -  (((ct*(ct - 1.*rh))/(r**2 + rh**2) + 
     -       (-1. + ct/Sqrt(r**2 + rh**2))**2)**2 + 
     -    (ct*(-1.*ct + rh)*(-1.*rh + Sqrt(r**2 + rh**2))*
...
c this is the metric function W

	 fW= (Sqrt(ct*(ct - 1.*rh))*(-2.*ct + rh)*
     -    (1. - (1.*ct)/Sqrt(r**2 + rh**2)))/
     -  ((r**2 + rh**2)**1.5*
     -    (((ct*(ct - 1.*rh))/(r**2 + rh**2) + 
     -         (1. - (1.*ct)/Sqrt(r**2 + rh**2))**2)**2 + 
     -  (ct*(-1.*ct + rh)*(1. - (1.*rh)/Sqrt(r**2 + rh**2))*
     -         sn**2)/(r**2 + rh**2)))
 
cccccccccccccccccccccccccccccccccccccccccccc

      expr1= ((r**2 + rh**2)*(rh + Sqrt(r**2 + rh**2))**2*
     -    (((ct*(ct - rh))/(r**2 + rh**2) + 
     -         (-1 + ct/Sqrt(r**2 + rh**2))**2)**2 + 
     -      (ct*(-ct + rh)*(-rh + Sqrt(r**2 + rh**2))*sn**2)/
     -       (r**2 + rh**2)**1.5))/r**2

      expr2=1. + rh/Sqrt(r**2 + rh**2)


	expr3=2.*r + (rh*Sqrt(r**2 + rh**2))/r


	expr4=  ((cs**2*ct*(ct - 1.*rh))/(r**2 + rh**2) + 
     -     (-1. + ct/Sqrt(r**2 + rh**2))**2)**2/
     -  (((ct*(ct - 1.*rh))/(r**2 + rh**2) + 
     -       (-1. + ct/Sqrt(r**2 + rh**2))**2)**2 + 
     -(ct*(-1.*ct + rh)*(-1.*rh + Sqrt(r**2 + rh**2))*sn**2)/
     -     (r**2 + rh**2)**1.5)


	expr5= ct**2 + r**2 + cs**2*ct*(ct - 1.*rh) + rh**2 - 
     -  2.*ct*Sqrt(r**2 + rh**2)

c       write(6,*)r 

      eq1=      -(expr4*nr**2*U1) - 
     -  expr5*sn**2*U1*(c3 + 2*c2*U1**2 + 3*c1*U1**4) + 
     -  sn**2*(g*H*(-2*ff**3*U1x + ff**4*U1xx) + U1yy) + 
     -  H*(expr3*ff**2*sn**2*U1x + cs*expr2*sn*U1y + 
     -     expr1*sn**2*U1*(fW*nr + w)**2)
...
c      Td44        
 	...
 	T44=-(c3*U1**2)-(csc**2*em2F2*nr**2*U1**2)/g-c2*U1**4-
     -  c1*U1**6 - em2F1*(ff**4*H*U1x**2 + U1y**2/g) - 
     -  (em2F0*g*U1**2*(-(fW**2*nr**2) + w**2))/(H*r**2)

c      Td34

      T34= (-2*em2F0*g*nr*U1**2*(fW*nr + w))/(H*r**2)
 

c      if(Abs(U1) .le.  max ) go to 121
c	max=U1
c	r1=r
c121    continue
 

       WRITE(1,489)T44,T34,sqr*T44,sqr*T34
...
c          open(unit=1,name='sup.dat')
c         rewind(1)
c        Close(Unit=1) 

c         rewind(1)
c        open(unit=1,name='sup.dat') 
c         WRITE(1,489) max,r1	         
c        Close(Unit=1)
...
C**                                                               ***       
     ...
     &       UX(MV,NK),UXX(MV,NK),UY(MV,NK),UYY(MV,NK)               
      ...
      DOUBLE PRECISION   c1,c2,c3,nr,w,ct,rh,max,r1
...
C**                                                             ***     
     ...
     &               PUT(MT),PUX(MV),PUXX(MV),PUY(MV),PUYY(MV),max 
       Double precision sn,cs,csc,r,z,ff,f
...
c
     ...
     & g,H,expr1,expr2,expr3,expr4,expr5,fW
...
cccccccccccccccccccccccccccccccccccccccccccccccccc
       ...
       Vxy11=0
       Vyy11=sn**2
       Vx11=ff**2*(expr3 - 2*ff*g)*H*sn**2
       Vy11=cs*expr2*H*sn
       V11=
     & -(expr4*nr**2) + sn**2*(-(c3*expr5) - 
     & 3.*expr5*U1**2*(2.*c2 + 5*c1*U1**2)+expr1*H*(fW*nr + w)**2
     &)
