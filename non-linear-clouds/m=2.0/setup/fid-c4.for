...
c versiune cu eqs multiplicate cu (1-X2-Y2)*r^2*sin[t]^2
...
C**                                                             ***
	 ...
	 Double precision norm,rn,xn,min, ro,try,
...
c       write(6,*)r 
      ...
      eq1=  -((expr4*nr**2*U1)/H) - 
     -  (expr5*sn**2*U1*(c3 + 2*c2*U1**2 + 3*c1*U1**4))/H + 
     -  expr3*ff**2*sn**2*U1x + cs*expr2*sn*U1y + 
     -  expr1*sn**2*U1*(fW*nr + w)**2 + 
     -  sn**2*(U1yy/H + (-2*ff**3*U1x + ff**4*U1xx)*g)
...
cccccccccccccccccccccccccccccccccccccccccccccccccc
       ...
       Vxy11=0
       Vyy11=sn**2/H
       Vx11=ff**2*sn**2*(expr3 - 2*ff*g)
       Vy11=cs*expr2*sn
       V11=
     & (-(expr4*nr**2) + sn**2*(-(c3*expr5) - 
     &        3*expr5*U1**2*(2*c2 + 5*c1*U1**2) + 
     &        expr1*H*(fW*nr + w)**2))/H
